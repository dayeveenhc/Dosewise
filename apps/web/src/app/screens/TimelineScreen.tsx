...
export function TimelineScreen({ patient, justAddedMed, onSendReminder }: { patient: Patient; justAddedMed?: string | null; onSendReminder: (medName?: string) => void }) {
  ...
  return (
    <div className="px-4 py-5">
      ...
      {/* Week strip */}
      <div className="bg-card rounded-2xl border border-border px-3 pt-3 pb-2 mb-4" data-walk="cg-week-strip">
          ...
          {dayMeds.map((med, i) => {
            ...
            return (
              <div key={med.id} className="flex gap-4 mb-1">
                ...
                <div data-testid={med.medicationId ? `medication-${med.medicationId}` : undefined} className={`flex-1 mb-4 bg-card rounded-2xl border shadow-sm overflow-hidden ${justAdded ? "border-2 border-emerald-400 ring-2 ring-emerald-300/50" : cfg.line}`}>
