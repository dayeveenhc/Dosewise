      ...
      <Card className="p-3.5" data-tour="cg-dashboard">
        ...
        <div className="flex flex-wrap gap-2 mt-1">
          {patient.medications.map(m => (
            <div key={m.id} title={`${m.name} · ${m.time} · ${m.status}`}>
        ...
        {colourBlind && (
          <div className="mt-1.5 space-y-0.5">
            {[...new Map(patient.medications.map(m => [m.name, m])).values()].map(m => {
              const shape = MED_SHAPES[m.name];
              return shape ? <p key={m.name} className="text-xs text-muted-foreground">{m.name}: {shape.shape} · {shape.marking}</p> : null;
            })}
          </div>
        )}
