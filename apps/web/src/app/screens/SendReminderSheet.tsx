...
export function SendReminderSheet({ patientName, medName, onClose, onSend }: SendReminderSheetProps) {
  ...
  const defaults = medName
    ? [
        `Please take your ${medName} now.`,
        "Just checking in — have you taken your medicine today?",
    ...
    : [
        ...
        "Just checking in — have you taken your medicine today?",
        "Don't forget your dose today.",
        "Please remember your next dose.",
      ];
      ...
      <div className="relative bg-card rounded-t-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85%]">
        ...
        {sent ? (
          <div className="flex flex-col items-center justify-center gap-3 px-6 py-12">
            ...
            <p className="font-['Fraunces'] text-lg font-semibold text-foreground">Reminder sent!</p>
            <p className="text-sm text-muted-foreground text-center">{patientName} will see it in their app.</p>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between px-5 pb-3 pt-1 border-b border-border shrink-0">
              <div>
                <h2 className="font-['Fraunces'] text-lg font-semibold text-foreground">Send reminder</h2>
                <p className="text-xs text-muted-foreground">To {patientName}</p>
              </div>
            ...
            <div className="overflow-y-auto scrollbar-none px-5 py-4 space-y-4">
              <div>
                <p className="text-xs font-semibold text-foreground mb-2">Quick options</p>
              ...
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1.5">Or write your own</label>
                <textarea
                  value={text}
