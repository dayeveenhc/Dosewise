...
import { Send, TrendingUp, Plane, Check, Sparkles, ChevronUp, Camera, FileText, Pill, Globe, Mic, Volume2, X, AlertTriangle, Brain, Circle, Trash2, Plus } from "lucide-react";
...
export function AskMeiScreen({ patient, elderId, onUpdatePatient, onNavigate, onMedsChanged, onMedAdded, doctorQuestions, onAddDoctorQ, onMarkAnswered, onDeleteQuestion }: {
  patient: Patient;
  elderId?: string;
  ...
  const buildGreeting = (): ChatMsg => ({
    id: 1, role: "agent",
    text: t(language, "ai.greeting", { name: patient.name }),
    time: nowLabel(),
  });
  // Key the saved chat by the signed-in user so each account restores its own
  // conversation, distinct from the elder shell's `mei-chat` key.
  const storageKey = `askmei-chat:${elderId ?? "anon"}`;
  const restored = loadChatSession(storageKey);
  const startedAtRef = useRef<number>(restored?.startedAt ?? Date.now());
  const [messages, setMessages] = useState<ChatMsg[]>(() => restored?.messages ?? [buildGreeting()]);
  ...
  // Force-expire the session after SESSION_TTL_MS even if the caregiver never
  ...
  const pushAgent = (text: string) => {
    setMessages(prev => [...prev, { id: Date.now(), role: "agent", text, time: nowLabel() }]);
    scrollToBottom();
    speak(text);
  };
  ...
  const handleSetup = () => {
    setQuickOpen(false);
    pushAgent(t(language, "ai.setupIntro", { nick: patient.nickname }));
  };
        ...
        <div className="absolute inset-0 z-[140] flex items-end p-3">
          ...
          <div className="relative w-full bg-card rounded-3xl border border-border px-4 pt-4 pb-4 shadow-2xl max-h-full overflow-y-auto scrollbar-none animate-in slide-in-from-bottom duration-200">
            ...
            <div className="grid grid-cols-2 gap-2">
              {/* Weekly Summary & Travel open their own sheets — close the popup
                  first, or it stays stacked on top of whatever they open. */}
