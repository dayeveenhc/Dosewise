...
// Pinned to Margaret's 8:30 AM doses, so this scripted scenario always shows
// them as "next" (blue) and logs at that same time, rather than drifting to
// "missed" — or logging the real wall-clock time — depending on when it's
// demoed. Paired with the ">=" below (not ">") so a dose scheduled for
// exactly "now" still reads as upcoming rather than already missed.
const DEMO_NOW: string | null = "8:30 AM";
...
export function WalkthroughHomeScreen({ patient, onLogDose, onOpenTravel, justAddedMed, demoToday, travelOffsetMinutes }: {
  patient: Patient;
  onLogDose: (id: number, takenAt?: string) => void;
  onOpenTravel: () => void;
  justAddedMed?: string | null;
  // Set once a travel plan is saved: jumps straight to that day (fully
  // interactive, as if it were "today") with dose times read against the
  // destination's wall-clock, not Singapore's.
  demoToday?: Date;
  travelOffsetMinutes?: number;
}) {
  const { colourBlind } = useAccessibility();
  const { language } = useLanguage();
  const [now, setNow] = useState(() => demoToday ?? makeNow());
  const [selectedDay, setSelectedDay] = useState<Date>(() => demoToday ?? new Date());
  // Shifts a resolved dose time to the destination's wall-clock reading —
  // `now` above is built the same way, so comparisons between the two stay
  // correct (both offset by the same delta) while everything displayed
  // (badges, hour buckets, the "now" line) reads in local trip time.
  const travelOffset = travelOffsetMinutes ?? 0;
  const resolveDoseLocal = (m: Medication) => {
    const r = resolveDose(m);
    if (!travelOffset) return r;
    const minutes = ((r.minutes + travelOffset) % 1440 + 1440) % 1440;
    return { ...r, minutes, clock: minutesToClock(minutes) };
  };
  ...
  useEffect(() => {
    if (demoToday) {
      // A travel-mode save (including re-saving from an already-mounted Home
      // tab, e.g. reopening via its own travel banner) — jump straight to
      // that day/time rather than waiting for a remount to pick it up, and
      // re-snap the scroll position since scrolledForDayRef may already
      // match if the date didn't change but the time did.
      setNow(demoToday);
      setSelectedDay(demoToday);
      scrolledForDayRef.current = "";
      return;
    }
  ...
  const statusForDay = (m: Medication, day: Date): MedStatus => {
    if (isToday(day)) {
      ...
      return resolveDoseLocal(m).minutes >= nowMinutes ? "upcoming" : "missed";
    }
  ...
  const renderCard = (m: Medication & { status: MedStatus }, vague: boolean, note?: string) => {
    ...
    const clock = resolveDoseLocal(m).clock;
        ...
        <div className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {vague && <p className="text-xs text-primary/80 mt-1">🕒 {t(language, "home.vagueTimeNote", { note: note ?? "", clock: minutesToClock(resolveDoseLocal(m).minutes) })}</p>}
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden relative">
      {/* Travel Mode indicator. Unlike the live app's version, the schedule
          below actually does shift to the destination timezone here — see
          demoToday/travelOffsetMinutes and resolveDoseLocal above. */}
          ...
          <div className="w-full bg-background rounded-t-3xl p-5 pb-7 animate-in slide-in-from-bottom duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t(language, "home.scheduledFor", { clock: resolveDoseLocal(pendingDose).clock })}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              />
              <button onClick={() => setTakenInput(to24hInput(now))} className="px-4 py-3 rounded-xl bg-muted text-sm font-bold text-foreground active:bg-muted/70">
