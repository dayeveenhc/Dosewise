...
import { WalkthroughHomeScreen } from "./WalkthroughHomeScreen";
import { WalkthroughPrescriptionScreen } from "./WalkthroughPrescriptionScreen";
import { WalkthroughAIScreen } from "./WalkthroughAIScreen";
import { WalkthroughNotificationsScreen } from "./WalkthroughNotificationsScreen";
import { WalkthroughSettingsScreen } from "./WalkthroughSettingsScreen";
import { WalkthroughAddPrescriptionSheet, type MedUpdate } from "./WalkthroughAddPrescriptionSheet";
import { WalkthroughTravelModeSheet } from "./WalkthroughTravelModeSheet";
...
// Margaret's home base for every seeded dose time in this scenario.
const HOME_OFFSET_MINUTES = 8 * 60; // Singapore, UTC+8

// TIMEZONES entries read "Name (UTC+9)" / "UTC-8" / "UTC+5:30" — pull the
// signed offset out so destination dose times can be shifted against home.
function parseUtcOffsetMinutes(tz: string): number {
  const m = tz.match(/UTC([+-])(\d+)(?::(\d+))?/);
  if (!m) return HOME_OFFSET_MINUTES;
  const sign = m[1] === "-" ? -1 : 1;
  return sign * (Number(m[2]) * 60 + Number(m[3] || 0));
}

// Real "now" isn't part of this fixed-moment demo, so day 4 of the trip is
// computed from the plan's own start date, at Margaret's usual 8:30 AM —
// shifted to read as the equivalent destination wall-clock time.
function computeTravelJump(plan: { startDate: string; endDate: string; timezone: string }) {
  const offsetMinutes = parseUtcOffsetMinutes(plan.timezone) - HOME_OFFSET_MINUTES;
  const day4 = new Date(`${plan.startDate}T08:30:00`);
  day4.setDate(day4.getDate() + 3);
  day4.setMinutes(day4.getMinutes() + offsetMinutes);
...
export function WalkthroughApp({ patient, elderId, onUpdatePatient, onBack, onSignOut, startTour, careMessages, initialDemoToday }: {
  ...
  // Lets a different scenario (e.g. walkthrough 2) pin this shell to its own
  // fixed moment instead of the 8:30 AM default below — reuses the exact
  ...
  // real wall clock — this scenario always plays out at the same fixed
  // moment, regardless of when it's actually demoed — unless a caller
  // hands us a different fixed moment via initialDemoToday.
  const [currentTime] = useState(() => {
    if (initialDemoToday) return initialDemoToday;
    const d = new Date();
    d.setHours(8, 30, 0, 0);
    return d;
  });
  ...
  // Which scripted "Add refill / prescription" scan is staged next: 0 = the
  // Metformin dosage change (crumpled label, folds into the existing card),
  // 1 = the new Latanoprost eye drops (clean label, added fresh at 9 PM),
  // 2 = collecting the refill Travel Mode warned her about (clean label,
  // tops the same card back up to a full supply). Persists across
  // opening/closing the sheet within one demo run, resets when the whole
  // scenario resets (this component remounts then).
  const [scanStep, setScanStep] = useState(0);
  const [showTravel, setShowTravel] = useState(false);
  // Set once a travel plan is saved: jumps the Home schedule to day 4 of the
  // trip, with dose times shifted to the destination's wall-clock time. Seeded
  // from initialDemoToday (no offset — same timezone) when the caller wants
  // the Home schedule pinned to a fixed moment from the very first render.
  const [travelJump, setTravelJump] = useState<{ demoToday: Date; offsetMinutes: number } | null>(
    () => initialDemoToday ? { demoToday: initialDemoToday, offsetMinutes: 0 } : null
  );
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
        {tab === "home"          && <WalkthroughHomeScreen         patient={patient} onLogDose={handleLogDose} onOpenTravel={() => setShowTravel(true)} justAddedMed={justAddedMed} demoToday={travelJump?.demoToday} travelOffsetMinutes={travelJump?.offsetMinutes} />}
        {tab === "prescriptions" && <WalkthroughPrescriptionScreen patient={patient} onOpenAI={openAI} onAddRx={() => setAddRx("manual")} justAddedMed={justAddedMed} />}
        {tab === "ai"            && (
          <WalkthroughAIScreen
            patient={patient}
            elderId={elderId}
      ...
      {addRx && (
        <WalkthroughAddPrescriptionSheet
          initialTab={addRx}
          routine={{ ...patient.mealTimes, sleepTime: patient.sleepTime }}
          existingMeds={patient.medications}
          onUpdateMed={handleUpdateMed}
          step={scanStep}
          onAdvanceStep={() => setScanStep(s => Math.min(2, s + 1))}
          onClose={() => setAddRx(null)}
          onAdd={handleAddPrescription}
          onAdded={() => setTab("prescriptions")}
          onAgentAdded={(name?: string) => { void refreshMeds(); flagJustAdded(name); setTab("prescriptions"); }}
        />
      )}
      {showTravel && (
        <WalkthroughTravelModeSheet
          patient={patient}
          onClose={() => setShowTravel(false)}
          onSaved={plan => {
            onUpdatePatient({ ...patient, travelPlan: plan });
            setTravelJump(plan ? computeTravelJump(plan) : null);
          }}
        />
      )}
