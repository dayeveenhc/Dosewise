...
async def _repl() -> None:
    ...
    if not llm.api_key_present(settings):
        print(
            f"{llm.api_key_env_name(settings)} is not set in .env — cannot start.",
            file=sys.stderr,
        )
        return
    ...
    print(f"Hermes CLI ({llm.provider(settings)}) — acting as elder {state.elder_id}")
    ...
    try:
        while True:
            ...
            reply, tools_used, state.messages = await run_agent_turn(
                client, ctx, text, history=state.messages
            )
            if tools_used:
